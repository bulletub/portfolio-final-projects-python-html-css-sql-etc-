...
if (isset($_POST['save_payment'])) {
    $method = $_POST['preferred_payment'] ?? '';
    $stmt = $pdo->prepare("ALTER TABLE users ADD COLUMN IF NOT EXISTS preferred_payment_method VARCHAR(50) NULL");
    try { $stmt->execute(); } catch (Throwable $e) { /* ignore if not supported */ }
    try {
        $upd = $pdo->prepare("UPDATE users SET preferred_payment_method=? WHERE id=?");
        $upd->execute([$method, $user_id]);
        $_SESSION['success'] = "Payment preferences saved.";
    } catch (Throwable $e) {
        $_SESSION['error'] = "Failed to save payment preference.";
    }
    header("Location: user_settings.php");
    exit;
}
...
function setActive(sectionId) {
  ...
  if (sectionId === 'addresses') {
    // if map exists, force reflow so tiles render correctly after show
    setTimeout(()=>{ try { if (window.__addrMap) window.__addrMap.invalidateSize(); } catch(e){} }, 150);
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Initialize map if addresses section exists
  const mapDiv = document.getElementById('map');
  if (mapDiv && window.L) {
    const map = L.map('map', { zoomControl:true }).setView([14.5995, 120.9842], 12); // Manila default
    window.__addrMap = map;
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
      attribution: '&copy; OpenStreetMap',
      crossOrigin: true
    }).addTo(map);
    let marker = null;
    const latEl = document.getElementById('address_lat');
    const lngEl = document.getElementById('address_lng');
    const addrEl = document.getElementById('address');

    async function setMarker(lat, lng, doReverse) {
      ...
      if (doReverse) {
        try {
          ...
          if (d && d.display_name) {
            addrEl.value = d.display_name;
          }
        } catch(err) {}
      }
      marker.on('dragend', async (e)=>{
        const ll = e.target.getLatLng();
        await setMarker(ll.lat, ll.lng, true);
      });
    }
    ...
    addrEl.addEventListener('input', () => {
      ...
      typingTimer = setTimeout(async ()=>{
        ...
        try {
          ...
          const d = await r.json();
          if (d && d[0]) setMarker(parseFloat(d[0].lat), parseFloat(d[0].lon), false);
        } catch(err) {}
      }, 500);
    });
    // ensure proper sizing on load and window resize
  ...
  <div class="max-w-7xl mx-auto p-6 flex space-x-8 items-start">
    ...
    <aside id="sidebar" class="w-64 bg-white rounded-lg shadow p-5">
      ...
      <nav class="flex flex-col space-y-2">
        ...
        <a href="#purchases" class="px-4 py-3 rounded hover:bg-orange-50 text-gray-700">🧾 Past Purchases</a>
        <a href="#payments" class="px-4 py-3 rounded hover:bg-orange-50 text-gray-700">💳 Payment Methods</a>
      </nav>
    </aside>
    ...
    <main class="flex-1">
      ...
      <!-- Past Purchases -->
      <section id="purchases" class="bg-white p-6 rounded-lg shadow hidden">
        <h2 class="text-xl font-semibold mb-6 border-b pb-2">Past Purchases</h2>
        <?php
          $orders = $pdo->prepare("SELECT id, status, created_at FROM order_groups WHERE user_id=? ORDER BY created_at DESC LIMIT 50");
          $orders->execute([$user_id]);
          $og = $orders->fetchAll(PDO::FETCH_ASSOC);
        ?>
        <?php if ($og): ?>
          <div class="divide-y">
            <?php foreach($og as $g): ?>
              <div class="py-3 flex items-center justify-between">
                <div>
                  <div class="font-medium">Order #<?= (int)$g['id'] ?></div>
                  <div class="text-xs text-gray-500"><?= date('M d, Y g:i A', strtotime($g['created_at'])) ?> • <?= htmlspecialchars(ucfirst($g['status'])) ?></div>
                </div>
                <div>
                  <a href="orders.php?open=<?= (int)$g['id'] ?>" class="px-3 py-1 text-sm rounded bg-orange-100 text-orange-700 hover:bg-orange-200">View</a>
                </div>
              </div>
            <?php endforeach; ?>
          </div>
        <?php else: ?>
          <p class="text-gray-500">No purchase history yet.</p>
        <?php endif; ?>
      </section>

      <!-- Payment Methods -->
      <section id="payments" class="bg-white p-6 rounded-lg shadow hidden">
        <h2 class="text-xl font-semibold mb-6 border-b pb-2">Payment Methods</h2>
        <?php
          $pref = $pdo->prepare("SELECT preferred_payment_method FROM users WHERE id=?");
          $pref->execute([$user_id]);
          $preferred = ($pref->fetch(PDO::FETCH_ASSOC)['preferred_payment_method'] ?? '');
        ?>
        <form method="post" class="space-y-4">
          <div>
            <label class="block text-sm text-gray-600 mb-1">Preferred Payment</label>
            <select name="preferred_payment" class="w-full border rounded px-3 py-2 focus:ring-1 focus:ring-orange-500">
              <option value="">Choose...</option>
              <option value="cod" <?= $preferred==='cod'?'selected':'' ?>>Cash on Delivery</option>
              <option value="paypal" <?= $preferred==='paypal'?'selected':'' ?>>PayPal</option>
              <option value="gcash" <?= $preferred==='gcash'?'selected':'' ?>>GCash</option>
            </select>
          </div>
          <button type="submit" name="save_payment" class="px-5 py-2 bg-orange-500 text-white rounded hover:bg-orange-600">Save</button>
        </form>
      </section>
<!-- Password -->
      ...
      <section id="addresses" class="bg-white p-6 rounded-lg shadow hidden">
        ...
        <form method="post" class="space-y-5 mt-6" id="addressForm">
          ...
          <div>
            ...
            <textarea name="address" id="address" class="w-full border rounded px-3 py-2 focus:ring-1 focus:ring-orange-500" placeholder="Start typing your address..."></textarea>
            <div id="map" class="mt-3 rounded border" style="height:220px;"></div>
