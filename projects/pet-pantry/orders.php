            ...
            <div class="p-3 border-b">
              <div class="flex items-center gap-4">
                ...
                <div class="flex-1">
                  ...
                  <div><?= $currencySymbol ?><?= number_format($item['price'],2) ?> × <?= $item['quantity'] ?></div>
                </div>
              </div>
    ...
    <form method="post">
      ...
      <div class="mt-4 flex justify-end gap-2">
        ...
        <button type="button" class="btn-red px-3 py-1 rounded" onclick="closeModal('refund-<?= $item['order_id'] ?>')">Cancel</button>
          ...
          $discount = floatval($orderGroup['info']['discount_amount'] ?? 0);
          $promoCode = $orderGroup['info']['promo_code'] ?? null;
          ...
          <div class="px-4 py-3 bg-gray-50 rounded-b-lg">
            ...
            <div class="flex justify-between items-center">
              <span class="font-bold text-lg">Total: <?= $currencySymbol ?><?= number_format($orderTotal,2) ?></span>
              <div class="flex gap-2">
                <button class="btn-orange px-4 py-2 rounded" onclick="openInvoiceModal(<?= $groupId ?>)">Invoice</button>
