...
if (isset($_POST['update_id'], $_POST['quantity'])) {
    ...
    // Get product stock
    $stmt = $conn->prepare("SELECT p.stock FROM cart c 
...
// ---------------------------
// Always return cart count
// ---------------------------
$countRes = $conn->prepare("SELECT COALESCE(SUM(quantity),0) AS cart_count FROM cart WHERE user_id=?");
...
echo json_encode([
    ...
    'message' => $responseMsg ?: "Cart status updated"
]);
